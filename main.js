...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    icon: __dirname + "./assets/yalda.ico",
  });
...
ipcMain.handle("search-links", async (event, { type, query }) => {
  ...
  const formattedQueryForFmovies = query
    .trim()
    .replace(/\s+/g, "+")
    .toLowerCase();
  ...
  const m4uhdLink = `https://ww2.m4uhd.tv/search/${query
    .replace(/\s+/g, "-")
    .toLowerCase()}.html`;
  ...
  for (const url of urls) {
    try {
      ...
      if (
        html.toLowerCase().includes(query.replace(/\s+/g, ".").toLowerCase())
      ) {
        matchingLinks.push(url);
      }
    } catch (error) {
